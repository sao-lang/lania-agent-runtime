...
> **核心原则**：`AgentRuntime` 是纯壳，不感知 tools / mcp / skills 等具体组件。
> `RuntimeBuilder` 负责接线，用户也可纯手动注册。

### 7.1 接线逻辑全在 Builder

`AgentRuntime.__init__` 不再接受 tools / mcp / skills 参数。
所有工具相关的接线在 `RuntimeBuilder.build()` 中完成：
...
class RuntimeBuilder:
    def build(self) -> AgentRuntime:
        ...
        # 2. 将 dispatcher.dispatch 设为 tool_executor
        runtime._tool_executor = dispatcher.dispatch

        # 3. 注册 before_llm Transform（自动刷新 tools_schema）
        async def inject_tools_schema(data, ctx):
            ctx.services["tools_schema"] = [
                t.to_openai_schema() for t in dispatcher.all_tools()
            ]
            return data

        runtime._hooks.register(
            HookPoint.BEFORE_LLM, inject_tools_schema,
            primitive=PrimitiveType.TRANSFORM,
            name="_tools_schema_refresh", priority=100,
        )

        # 4. 注册 Skill before_llm hook
        if skill_manager:
            runtime._hooks.register(
                HookPoint.BEFORE_LLM, skill_manager.get_before_llm_hook(),
                primitive=PrimitiveType.TRANSFORM,
...
用户也可以完全手动接线，不经过 Builder：

```python
from src.runtime import AgentRuntime, HookRegistry, HookPoint, PrimitiveType
from src.tools import ToolRegistry, ToolDispatcher

# 1. 创建工具
tools = ToolRegistry()
tools.register(ToolSpec(name="calc", handler=calc_handler, ...))
dispatcher = ToolDispatcher(tool_registry=tools)

# 2. 手动注册 before_llm Transform
hooks = HookRegistry()
hooks.register(
    HookPoint.BEFORE_LLM, inject_tools_schema,
    primitive=PrimitiveType.TRANSFORM, name="inject_tools",
)

# 3. Runtime 纯壳——不 import src.tools
runtime = AgentRuntime(
    system_prompt="你是助手",
    hooks=hooks,
    llm_executor=my_executor,
    tool_executor=dispatcher.dispatch,  # ← 直接传入 dispatch 函数
)
```

### 7.3 使用示例（Builder 快捷方式）
...
before_llm:
  ...
  2. tools_schema_refresh → 合并 Tool + MCP schema
...
```python
from src.runtime import AgentRuntime
from src.tools import ToolRegistry, ToolSpec, MCPServerManager, MCPServerConfig
...
```python
from src.runtime import AgentRuntime
from src.tools import ToolRegistry, ToolSpec, MCPServerManager, SkillManager

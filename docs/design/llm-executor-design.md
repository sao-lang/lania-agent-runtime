...
> ⚠️ 下图中标注的 MemoryRecall、RAGRetrieval、TokenManager、MemoryCommit 等
> 均为**用户可选注册的 Hook 示例**，并非 Runtime 内置功能。
> Runtime 仅内置 `_tools_schema_refresh`（before_llm）和 `_default_budget`（after_llm）
> 两个 Transform。其余所有治理和扩展行为均由用户通过 Hook 机制注入。
...
class AgentRuntime:
    """
    Agent Runtime 外部接口——纯壳，不感知任何外部组件。

    使用方式:
      runtime = AgentRuntime(system_prompt="你是助手", llm_executor=..., tool_executor=...)
...
```python
async def cli():
    runtime = AgentRuntime(system_prompt="你是助手", llm_executor=executor, ...)
    while True:
        user_input = input("> ")
...
> ⚠️ Runtime 是纯壳。所有非核心行为（tools schema、memory、context assembly）
> 都是 Builder 或用户注册的 Hook，Runtime 自身不内置任何外部组件。
...
run("你好")
    ...
    └─→ ctx.messages.append({role:"user", content:"你好"})
         │
         ▼
       [before_step]   →  Transform (用户注册：审计日志等)
       [before_llm]     →  Transform: _tools_schema_refresh (Builder 注册)
                           Transform: ContextAssemblerHook (Builder 注册，可选)
                           Intercept: Input guardrails (用户注册，可选)
       [serialize]      →  contextPayload → messages[0]
       [llm_executor]   →  return LLMResponse (用户传入)
       [Runtime]        →  messages.append({role:"assistant", ...})
       [after_llm]      →  Intercept/Observe (用户注册，可选)
                           Transform: _default_budget (Runtime 内置)
       [after_step]     →  Transform: MemoryCommitHook (Builder 注册，可选)
...
Runtime **唯一内置**的 Transform 是 `_default_budget`（token 用量记账）。
`_tools_schema_refresh`、`ContextAssemblerHook`、`MemoryCommitHook` 等
均由 Builder 在 `build()` 时注册——用户也可纯手动注册。
...
class TestFullChain:
    ...
    async def test_guardrails_block_then_recover(self):
        ...
        from src.runtime import HookRegistry
        from src.runtime._types import HookPoint, PrimitiveType

        hooks = HookRegistry()
        hooks.register(HookPoint.BEFORE_LLM, InputGuardrails(), primitive=PrimitiveType.INTERCEPT)
        hooks.register(HookPoint.ON_ERROR, ErrorRecoveryHook(), primitive=PrimitiveType.TRANSFORM)
